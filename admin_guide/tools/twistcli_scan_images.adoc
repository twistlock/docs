...
If you have Login whitelisting feature enabled in Prisma Cloud, you will need to perform some additional steps in order to perform scans or run API calls against Compute Console.

==== Step 1: Get a token from /login API
...
==== Step 2: Use the token1 from above in /config API
...
==== Step 3: Use the token2 from above in /authenticate API
...
==== Step 4: Use the token3 from above in twistcli or API usage:
