...
ifdef::prisma_cloud[]
. Download the https://cdn.twistlock.com/docs/attachments/amazon-ecs-task-tw-defender-saas.json[Prisma Cloud Defender task definition],
endif::prisma_cloud[]
...
.. (Optional) Change the task definition name before creating. The JSON will default the name to *twistlock_defender*.
