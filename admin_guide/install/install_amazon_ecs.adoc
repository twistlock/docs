...
. The following command will generate certificates needed for Defender connection.
...
.. Set the ownership and permissions for each file.
  ...
  $ chown root:root ca.pem client-cert.pem client-key.pem server-cert.pem server-cert.key service-parameter
  $ chmod 600 ca.pem client-cert.pem client-key.pem server-cert.pem server-cert.key service-parameter
...
[.procedure]
. Download the https://cdn.twistlock.com/docs/attachments/amazon-ecs-task-tw-defender-20.json[Prisma Cloud Defender task definition], and open it for editing.
