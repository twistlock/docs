...
[NOTE]
====
If a user is assigned multiple system roles, either directly or through group inheritance, then he is granted with the higest role, and access to the assigned collections of all the groups he belongs to. If a user is assigned both system and custom roles, then he will be randomly granted the rights of one of the groups, including its role and assigned collections.
====
