...
When using AWS STS, make sure the following apply:

. The policy of the IAM user you use as credentials have *sts:AssumeRole* permission on the IAM role you're going to assume. Sample policy:
+
[source,json]
----
{
