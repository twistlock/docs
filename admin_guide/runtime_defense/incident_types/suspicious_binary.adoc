...
Attackers use automated tools frequently to download multiple binaries of different architectures when the target architecture is not known. Process _curl_ downloading an ARM binary, for example, strongly indicates a breach had taken place.
...
The following incident shows that the ELF file _upx_ was written to the file system and is suspected as malicious. Its ELF header indicates using anti-analysis techniques to modify the file.
