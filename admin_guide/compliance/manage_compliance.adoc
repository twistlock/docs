...
As you configure your compliance policy, you'll want a deeper understanding of each check.
Teams that address compliance issues might also need more information about failing checks, so that they can resolve the underlying issues.
...
Our IDs map to the CIS benchmark IDs (in most cases).
For example, our ID 51 maps to CIS Docker Benchmark 5.1

Show screenshot that shows the origin of these compliance checks.

* All CRI checks are direct analogs of the Docker CIS checks, except for CRI environments.

* For all other _Twistlock Labs_ checks, we’ve got documentation (Twistlock Labs - Container/Image/Istio, Twistlock Labs - Windows host, Serverless). If customers are having any problems understanding the docs for these custom checks, let me know, so I can help to clarify them.

==== Table of compliance check prefixes.
