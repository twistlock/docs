...
For containers, Defender runs the compliance checks inside a restricted sanboxed container instantiated from the image being scanned, thus avoiding unncessary risk associated with running arbitarary code.

For hosts, Defedener runs the compliance checks on the host itself with unrestricted privileges to allow execution of any script. In order to limit the exposure only to customers that require this capability, this feature is disabled by default. To enable custom compliance checks for hosts, enable this setting first via *Manage > Defenders > Advanced Settings > Custom Compliance Checks for hosts* and deploy or redeploy Defenders if already deployed. Disabling this setting take place immediatly and does not require Defenders to be redepolyed. 
...
.. Go to *Defend > Compliance > Containers and Images* for containers or + 
*Defend > Compliance > Hosts* for hosts.
